...
local function StartCustomBarTicker()
  ...
  -- v2.9.2: Doubled interval from 0.1s to 0.2s for performance
  customBarTicker = C_Timer.NewTicker(0.2, function()
    local db = ns.API.GetDB()
    if not db or not db.bars then return end
...
local function StartDurationBarTicker()
  ...
  -- v2.9.2: Doubled interval from 0.06s to 0.12s for performance (8.3/sec instead of 17/sec)
  durationBarTicker = C_Timer.NewTicker(0.12, function()
...
local UPDATE_THROTTLE = 0.1  -- Max 10 updates/sec from events
