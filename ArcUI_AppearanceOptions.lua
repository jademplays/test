...
local function GetAllBarsDropdown()
  ...
  for i = 1, 30 do
    local cfg = ns.API.GetResourceBarConfig(i)
    if cfg and cfg.tracking.enabled then
      -- Check if power type is valid for current spec
      local powerType = cfg.tracking.powerType
      if not IsPowerTypeValidForSpec(powerType) then
        -- Skip - power not valid for spec
      else
        -- Check if bar should show on current spec
        local showOnSpecs = cfg.behavior and cfg.behavior.showOnSpecs
        local specAllowed = true
  ...
  if ns.db and ns.db.char and ns.db.char.cooldownBars then
    for i = 1, 10 do
      local cfg = ns.API.GetCooldownBarConfig(i)
...
local function UpdateBar()
  ...
  elseif barType:find("^cd_") then
    -- Cooldown bars don't need UpdateBar - ApplyAppearance handles it
  ...
  return {
    ...
    args = {
      ...
      barHeight = {
        ...
        order = 13,
        width = 1.0,
        -- Hide for charge bars since they use slotHeight instead
        hidden = function() return GetSelectedConfig() == nil or IsIconMode() or IsChargeBar() or collapsedSections.barSize end
      },
      barOpacity = {
        type = "range",
        name = "Bar Opacity",
        ...
        set = function(info, r, g, b, a)
          ...
          if cfg then
            if barType == "buff" then
              ...
              -- Also update thresholds[1] for granular modes
      ...
      -- SLOT BACKGROUND COLOR (Charge bars only)
      slotBackgroundColor = {
        type = "color",
        name = "Slot Background",
        desc = "Background color of charge slots (charge bars only)",
        hasAlpha = true,
        get = function()
          local cfg = GetSelectedConfig()
      ...
      enableFolded = {
        ...
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then
            cfg.display.thresholdMode = value and "folded" or "simple"
            UpdateBar()
          end
        end,
        order = 32,
      ...
      enableThresholds = {
        type = "toggle",
        name = "Thresholds",
        desc = "Change bar color at different value thresholds",
        get = function()
          local cfg = GetSelectedConfig()
        ...
        set = function(info, value)
          ...
          if cfg then
            if value then
              ...
              if not cfg.thresholds then
                cfg.thresholds = {
                  [1] = { enabled = true, minValue = 0, color = cfg.display.barColor or {r=0, g=0.5, b=1, a=1} },
                  [2] = { enabled = false, minValue = 5, color = {r=1, g=1, b=0, a=1} },
                  [3] = { enabled = false, minValue = 8, color = {r=0, g=1, b=0, a=1} },
                  [4] = { enabled = false, minValue = 50, color = {r=1, g=0.5, b=0, a=1} },
                  [5] = { enabled = false, minValue = 70, color = {r=0.5, g=0, b=1, a=1} },
                  [6] = { enabled = false, minValue = 90, color = {r=1, g=0, b=1, a=1} }
                }
              end
            else
              cfg.display.thresholdMode = "simple"
      ...
      threshold3Enable = {
        ...
        set = function(info, value)
          ...
          if cfg then
            if not cfg.thresholds then cfg.thresholds = {} end
            if not cfg.thresholds[3] then cfg.thresholds[3] = { enabled = false, minValue = 8, color = {r=0, g=1, b=0, a=1} } end
      ...
      threshold4Enable = {
        ...
        set = function(info, value)
          ...
          if cfg then
            if not cfg.thresholds then cfg.thresholds = {} end
            if not cfg.thresholds[4] then cfg.thresholds[4] = { enabled = false, minValue = 50, color = {r=1, g=0.5, b=0, a=1} } end
      ...
      threshold4Min = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return tostring(cfg and cfg.thresholds and cfg.thresholds[4] and cfg.thresholds[4].minValue or 50)
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[4] then
            cfg.thresholds[4].minValue = tonumber(value) or 50
            UpdateBar()
          end
        end,
        order = 33.8,
      ...
      threshold5Enable = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.thresholds and cfg.thresholds[5] and cfg.thresholds[5].enabled
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then
            if not cfg.thresholds then cfg.thresholds = {} end
            if not cfg.thresholds[5] then cfg.thresholds[5] = { enabled = false, minValue = 70, color = {r=0.5, g=0, b=1, a=1} } end
            cfg.thresholds[5].enabled = value
            UpdateBar()
          end
        end,
        order = 34.0,
      ...
      threshold5Min = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return tostring(cfg and cfg.thresholds and cfg.thresholds[5] and cfg.thresholds[5].minValue or 70)
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[5] then
            cfg.thresholds[5].minValue = tonumber(value) or 70
            UpdateBar()
          end
        end,
        order = 34.1,
        ...
        hidden = function()
          ...
          if IsDurationBar() then return true end
          local cfg = GetSelectedConfig()
          return not cfg or cfg.display.thresholdMode ~= "granular" or not (cfg.thresholds and cfg.thresholds[5] and cfg.thresholds[5].enabled)
        end
      },
      threshold5Color = {
        type = "color",
        ...
        get = function()
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[5] and cfg.thresholds[5].color then
            local c = cfg.thresholds[5].color
            return c.r, c.g, c.b, c.a or 1
          end
          return 0.5, 0, 1, 1  -- Purple
        end,
        set = function(info, r, g, b, a)
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[5] then
            cfg.thresholds[5].color = {r=r, g=g, b=b, a=a}
            RefreshBar()
          end
        end,
        order = 34.2,
      ...
      threshold6Enable = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.thresholds and cfg.thresholds[6] and cfg.thresholds[6].enabled
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then
            if not cfg.thresholds then cfg.thresholds = {} end
            if not cfg.thresholds[6] then cfg.thresholds[6] = { enabled = false, minValue = 90, color = {r=1, g=0, b=1, a=1} } end
            cfg.thresholds[6].enabled = value
            UpdateBar()
          end
        end,
        order = 34.3,
      ...
      threshold6Min = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return tostring(cfg and cfg.thresholds and cfg.thresholds[6] and cfg.thresholds[6].minValue or 90)
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[6] then
            cfg.thresholds[6].minValue = tonumber(value) or 90
            UpdateBar()
          end
        end,
        order = 34.4,
        ...
        hidden = function()
          ...
          if IsDurationBar() then return true end
          local cfg = GetSelectedConfig()
          return not cfg or cfg.display.thresholdMode ~= "granular" or not (cfg.thresholds and cfg.thresholds[6] and cfg.thresholds[6].enabled)
        end
      },
      threshold6Color = {
        type = "color",
        ...
        get = function()
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[6] and cfg.thresholds[6].color then
            local c = cfg.thresholds[6].color
            return c.r, c.g, c.b, c.a or 1
          end
        ...
        set = function(info, r, g, b, a)
          local cfg = GetSelectedConfig()
          if cfg and cfg.thresholds and cfg.thresholds[6] then
            cfg.thresholds[6].color = {r=r, g=g, b=b, a=a}
            RefreshBar()
          end
        end,
        order = 34.5,
      ...
      durationThresholdHeader = {
        ...
        order = 33.7,
        hidden = function()
          return true  -- Hidden - no longer needed
        end
      },
        ...
        set = function(info, value)
          ...
          if cfg then
            ...
            if ns.Display and ns.Display.ClearDurationColorCurve then
              local barType, barNum = GetSelectedBarType()
              if barNum then ns.Display.ClearDurationColorCurve(barNum) end
            end
            RefreshBar()
          end
        end,
      ...
      -- Charge bar specific: Frame Width (outer background width)
      frameWidth = {
        type = "range",
        name = "Frame Width",
        desc = "Width of the outer frame/background (charge bars only). Independent of slots width.",
      ...
      -- Charge bar specific: Frame Height (outer background height)
      frameHeight = {
        type = "range",
        name = "Frame Height",
        desc = "Height of the outer frame/background (charge bars only)",
      ...
      showBorder = {
        ...
        order = 51,
        width = 0.7,
        hidden = function() return GetSelectedConfig() == nil or IsIconMode() or collapsedSections.border end
      },
      useClassColorBorder = {
        type = "toggle",
        name = "Class Color",
      ...
      textAnchor = {
        ...
        hidden = function()
          if IsIconMode() or collapsedSections.stackText then return true end
          if IsChargeBar() then return true end  -- Hide for charge bars (use Charge Text Anchor instead)
          local cfg = GetSelectedConfig()
          return not (cfg and cfg.display.showText)
        end
      },
      textAnchorOffsetX = {
        ...
        hidden = function()
          if IsIconMode() or collapsedSections.stackText then return true end
          if IsChargeBar() then return true end  -- Hide for charge bars
          local cfg = GetSelectedConfig()
      ...
      chargeTextAnchor = {
        ...
        desc = "Position of the charge count text (e.g. 2/2). FREE allows dragging.",
        values = {
          ["FREE"] = "Free (Draggable)",
          ["TOPRIGHT"] = "Top Right",
          ["TOPLEFT"] = "Top Left",
        ...
        hidden = function()
          if IsIconMode() or collapsedSections.stackText then return true end
          local cfg = GetSelectedConfig()
          return not IsChargeBar() or not (cfg and cfg.display.showText)
        end
      },
      ...
      stackTextStrata = {
        type = "select",
        name = "Strata",
        desc = "Frame strata for charge text. Higher strata appears above lower strata.",
        values = {
          ["BACKGROUND"] = "BACKGROUND",
          ["LOW"] = "LOW",
          ["MEDIUM"] = "MEDIUM",
      ...
      durationShowWhenReady = {
        type = "toggle",
        name = "Show When Ready",
        desc = "Show duration as '0' when ability is not on cooldown. By default, duration is hidden when ready.",
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.display.durationShowWhenReady
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then
            cfg.display.durationShowWhenReady = value
            RefreshBar()
          end
        end,
        order = 76.36,
      ...
      durationTextStrata = {
        ...
        hidden = function()
          if IsIconMode() or collapsedSections.durationText then return true end
          local cfg = GetSelectedConfig()
          local showForBar = IsChargeBar() or IsCooldownDurationBar()
          return not showForBar or not (cfg and cfg.display.showDuration)
        end
      },
      -- Duration text level
      durationTextLevel = {
      ...
      -- Name text strata (for charge bars and cooldown duration bars)
      nameTextStrata = {
        type = "select",
        name = "Strata",
        ...
        hidden = function()
          ...
          local cfg = GetSelectedConfig()
          local showForBar = IsChargeBar() or IsCooldownDurationBar()
          return not showForBar or not (cfg and cfg.display.showName)
        end
      },
      -- Name text level
      nameTextLevel = {
