...
local function CreateArcAuraFrame(arcID, config)
    ...
    -- Count text (for charges/stacks) - parented to container for proper strata
    ...
    frame:SetScript("OnDragStop", function(self)
        ...
        -- Save to character DB (native Arc Auras storage - kept for backwards compat)
        ...
        if frame and frame:IsShown() then
            ...
            if config then
                ...
                if isOnCooldown and not isGlowPreview then
                    ...
                    -- Alpha (use stateVisuals if available, fallback to raw settings)
                    local cooldownAlpha = (stateVisuals and stateVisuals.cooldownAlpha) or cs.alpha or 1.0
                    ...
                    -- Alpha (use stateVisuals if available, fallback to raw settings)
                    local readyAlpha = (stateVisuals and stateVisuals.readyAlpha) or rs.alpha or 1.0
                    -- OPTIMIZED: Only call SetAlpha when value changes
                ...
                -- frame.Count is created in CreateArcAuraFrame, should always exist
                if frame.Count then
                    ...
                    if displayValue ~= nil and chargeTextEnabled then
                        ...
                        if not frame._arcStackStyleApplied then
                            ApplyStackTextStyle(frame, frame.Count)
                        ...
                        frame.Count:SetText(displayValue)
                        frame.Count:Show()
                    else
...
function ArcAuras.ApplySettingsToFrame(arcID, frame)
    ...
    -- Apply size with scale factored in (DON'T use SetScale - use SetSize instead)
    -- This matches how CDM icons work and avoids coordinate calculation issues
    ...
    if frame.Icon and padding > 0 then
...
function ArcAuras.RefreshAllFrames()
    ...
    for arcID, config in pairs(db.trackedItems or {}) do
        if config.enabled then
            local frame = ArcAuras.CreateFrame(arcID, config)
            if frame then frame:Show() end  -- Must show frames after creation!
        end
    end
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "PLAYER_EQUIPMENT_CHANGED" then
        ...
        if slot == 13 or slot == 14 then
            ...
            if frame then
                local config = frame._arcConfig
                ArcAuras.UpdateFrameIcon(frame, config)
                -- Reset stack style to re-apply with new trinket
                frame._arcStackStyleApplied = false
                -- Invalidate stack cache for this trinket
                InvalidateStackCache(arcID)
            end
        end
    ...
    elseif event == "PLAYER_SPECIALIZATION_CHANGED" or event == "ACTIVE_TALENT_GROUP_CHANGED" then
        -- NOTE: Don't call RefreshAllFrames() here!
        -- Arc Aura frames don't need to be destroyed/recreated on spec change.
        -- CDMGroups handles per-spec position restoration automatically.
        -- Destroying frames here causes a race condition where they get recreated
        -- before CDMGroups has loaded the new spec's savedPositions.
