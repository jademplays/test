...
local function CreateBarFrame(barNumber)
  ...
  -- Border lines (on tickOverlay with OVERLAY layer for crisp rendering)
  frame.borderLines = {}
  for i = 1, 4 do
    frame.borderLines[i] = frame.tickOverlay:CreateLine(nil, "OVERLAY")
  ...
  if barConfig.display.showTickMarks and maxValue > 1 then
    ...
    for _, tickValue in ipairs(tickPositions) do
      if barFrame.tickMarks and barFrame.tickMarks[tickIndex] then
        ...
        barFrame.tickMarks[tickIndex]:SetThickness(thickness)
...
function ns.Display.UpdateBar(barNumber, stacks, maxStacks, active, durationFontString, iconTexture)
    ...
    if cfg.iconMultiMode then
      ...
      if multiIconFrames[barNumber] then
        for i = maxStacks + 1, #multiIconFrames[barNumber] do
          if multiIconFrames[barNumber][i] then
            multiIconFrames[barNumber][i]:Hide()
          end
        end
      end
  ...
  elseif displayMode == "granularTest" then
    ...
    for i = #ranges + 1, #barFrame.stackedBars do
      if barFrame.stackedBars[i] then
        barFrame.stackedBars[i]:Hide()
      end
    end
...
function ns.Display.DeleteBar(barNumber)
  local cfg = ns.API and ns.API.GetBarConfig and ns.API.GetBarConfig(barNumber)
  if cfg then
    -- Clear tracking config (reset to defaults, don't set to nil)
    ...
    cfg.tracking.maxStacks = 10  -- Reset to default, not nil
...
function ns.Display.ShowDeleteButtons()
  ...
  for barNumber = 1, 30 do
    if barFrames[barNumber] then
      ...
      local barFrame = barFrames[barNumber].barFrame
      ...
      local iconFrame = barFrames[barNumber].iconFrame
...
function ns.Display.UpdateDurationBar(barNumber, stacks, maxStacks, active, sourceBar, stacksFontString, iconTexture)
  ...
  if barFrame.stackedBars then
    for i = 1, #barFrame.stackedBars do barFrame.stackedBars[i]:Hide() end
  end
  if barFrame.granularBars then
    for i = 1, #barFrame.granularBars do barFrame.granularBars[i]:Hide() end
  end
  if barFrame.durationGranularBars then
    for i = 1, #barFrame.durationGranularBars do barFrame.durationGranularBars[i]:Hide() end
  end
  if barFrame.durationLayers then
    for i = 1, #barFrame.durationLayers do barFrame.durationLayers[i]:Hide() end
  end
  if barFrame.durationStackedBars then
    for i = 1, #barFrame.durationStackedBars do barFrame.durationStackedBars[i]:Hide() end
  end
  if barFrame.durationLayeredBars then
    for i = 1, #barFrame.durationLayeredBars do barFrame.durationLayeredBars[i]:Hide() end
  end
  ...
  if needsSetup then
    ...
    barFrame.bar:SetFrameLevel(barFrame:GetFrameLevel() + 6)
...
function ns.Display.ApplyAppearance(barNumber)
  ...
  -- Borders are on tickOverlay for crisp rendering above fill
  if cfg.showBorder then
    local thickness = cfg.drawnBorderThickness
    local bc = cfg.borderColor
    local width = barFrame:GetWidth()
    local height = barFrame:GetHeight()
    
    -- Top border
    barFrame.borderLines[1]:ClearAllPoints()
    barFrame.borderLines[1]:SetStartPoint("TOPLEFT", barFrame.tickOverlay, 0, 0)
    barFrame.borderLines[1]:SetEndPoint("TOPLEFT", barFrame.tickOverlay, width, 0)
    barFrame.borderLines[1]:SetColorTexture(bc.r, bc.g, bc.b, bc.a)
    barFrame.borderLines[1]:SetThickness(thickness)
    barFrame.borderLines[1]:Show()
    
    -- Bottom border
    barFrame.borderLines[2]:ClearAllPoints()
    barFrame.borderLines[2]:SetStartPoint("BOTTOMLEFT", barFrame.tickOverlay, 0, 0)
    barFrame.borderLines[2]:SetEndPoint("BOTTOMLEFT", barFrame.tickOverlay, width, 0)
    barFrame.borderLines[2]:SetColorTexture(bc.r, bc.g, bc.b, bc.a)
    barFrame.borderLines[2]:SetThickness(thickness)
    barFrame.borderLines[2]:Show()
    
    -- Left border
    barFrame.borderLines[3]:ClearAllPoints()
    barFrame.borderLines[3]:SetStartPoint("TOPLEFT", barFrame.tickOverlay, 0, 0)
    barFrame.borderLines[3]:SetEndPoint("TOPLEFT", barFrame.tickOverlay, 0, -height)
    barFrame.borderLines[3]:SetColorTexture(bc.r, bc.g, bc.b, bc.a)
    barFrame.borderLines[3]:SetThickness(thickness)
    barFrame.borderLines[3]:Show()
    
    -- Right border
    barFrame.borderLines[4]:ClearAllPoints()
    barFrame.borderLines[4]:SetStartPoint("TOPRIGHT", barFrame.tickOverlay, 0, 0)
    barFrame.borderLines[4]:SetEndPoint("TOPRIGHT", barFrame.tickOverlay, 0, -height)
    barFrame.borderLines[4]:SetColorTexture(bc.r, bc.g, bc.b, bc.a)
    barFrame.borderLines[4]:SetThickness(thickness)
    barFrame.borderLines[4]:Show()
  else
    for i = 1, 4 do
      barFrame.borderLines[i]:Hide()
    end
  end
...
function ns.Display.RefreshAllBars()
  ...
  for barNumber = 1, 30 do
    -- CRITICAL: Only process bars that already exist in the database
    -- Do NOT use GetBarConfig here as it creates default configs!
