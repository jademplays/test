...
-- They have frame.totemData when active with totemData.slot being non-secret
-- ═══════════════════════════════════════════════════════════════════════════

-- Check if a frame has active totemData
...
local function GetTotemState(frame)
  ...
  local debugEnabled = ns.CDMGroups and ns.CDMGroups.debugEnabled
  local cdID = frame.cooldownID
  
  -- Simply check if totemData exists with a valid slot
  -- This works for totems, ground effects like Consecration, etc.
  local totemData = frame.totemData
  if totemData then
    local slotVal = totemData.slot
    if slotVal and type(slotVal) == "number" and slotVal > 0 then
      if debugEnabled then
        print("|cFF00FF00[GetTotemState]|r cdID:", cdID, "-> frame.totemData.slot =", slotVal)
      end
      return true, true, slotVal  -- hasTotemData=true, isActive=true, slot
    end
    -- Has totemData structure but no valid slot (shouldn't happen normally)
    return true, false, nil
  end
  
  -- FALLBACK: Check CDMGroups totemData cache if frame.totemData was wiped by pool reset
  if cdID and ns.CDMGroups and ns.CDMGroups.GetCachedTotemData then
    local cached = ns.CDMGroups.GetCachedTotemData(cdID)
    if cached and cached.slot and type(cached.slot) == "number" and cached.slot > 0 then
      -- Verify the totem is still active via game API
      local haveTotem, name, startTime, duration = GetTotemInfo(cached.slot)
      if haveTotem then
        if debugEnabled then
          print("|cFF00FFFF[GetTotemState]|r cdID:", cdID, "-> CACHE HIT! slot =", cached.slot, "totem active:", name)
        end
        return true, true, cached.slot  -- Cache is valid, totem still exists
      else
        if debugEnabled then
          print("|cFFFF8800[GetTotemState]|r cdID:", cdID, "-> Cache found but totem expired, slot:", cached.slot)
        end
      end
      -- Totem no longer exists, don't report as active
    else
      if debugEnabled and cdID then
        print("|cFFFF0000[GetTotemState]|r cdID:", cdID, "-> No frame.totemData, no cache entry")
      end
    end
  end
...
local function GetDB()
  ...
  if cdmGroupsDB and not cdmGroupsDB.migratedProfileIconSettings then
    ...
    if specData then
      ...
      -- Migrate iconSettings if profile has them and char spec doesn't
      if db.iconSettings and next(db.iconSettings) then
        if not specData.iconSettings or not next(specData.iconSettings) then
          for cdID, settings in pairs(db.iconSettings) do
            if not specData.iconSettings[cdID] then
              specData.iconSettings[cdID] = CopyTable(settings)
            end
          end
          print("|cff00ff00[ArcUI CDM]|r Migrated per-icon settings to character storage")
          didMigrate = true
        end
      end
...
local DEFAULT_ICON_SETTINGS = {
  ...
  -- Range Indicator
  rangeIndicator = {
    enabled = true,           -- Show out-of-range overlay
    alpha = 0.6,              -- Alpha of the overlay (0 = invisible, 1 = fully opaque)
    desaturate = false,       -- Also desaturate the icon when out of range
  },
...
local function GetOrCreateIconSettings(cooldownID)
  ...
  local cfg = iconSettings[key]
  
  -- Ensure sub-tables exist (but don't fill with defaults)
  if not cfg.border then cfg.border = {} end
  if not cfg.chargeText then cfg.chargeText = {} end
  if not cfg.cooldownText then cfg.cooldownText = {} end
  if not cfg.cooldownSwipe then cfg.cooldownSwipe = {} end
  if not cfg.auraActiveState then cfg.auraActiveState = {} end
  if not cfg.rangeIndicator then cfg.rangeIndicator = {} end
  if not cfg.procGlow then cfg.procGlow = {} end
  if not cfg.alertEvents then cfg.alertEvents = {} end
  if not cfg.alertEvents.onAvailable then cfg.alertEvents.onAvailable = {} end
  if not cfg.alertEvents.onPandemic then cfg.alertEvents.onPandemic = {} end
  if not cfg.alertEvents.onUnavailable then cfg.alertEvents.onUnavailable = {} end
  if not cfg.alertEvents.onChargeGained then cfg.alertEvents.onChargeGained = {} end
  if not cfg.cooldownStateVisuals then cfg.cooldownStateVisuals = {} end
  if not cfg.cooldownStateVisuals.readyState then cfg.cooldownStateVisuals.readyState = {} end
  if not cfg.cooldownStateVisuals.cooldownState then cfg.cooldownStateVisuals.cooldownState = {} end
  if not cfg.position then cfg.position = {} end
...
local function UpdatePreviewGlow(frame, cdID, cfg)
  if not LCG then return end  -- Use module-level LCG
  ...
  if showPreview then
    ...
    elseif glowType == "button" then
      local speed = glowCfg.speed or 0.125
      pcall(LCG.ButtonGlow_Start, frame, color, speed, 1)
    ...
    elseif glowType == "proc" then
      pcall(LCG.ProcGlow_Start, frame, {
        color = color,
        startAnim = false,  -- Don't play start animation repeatedly
        key = "ArcUI_Preview",
        frameLevel = 1,
        xOffset = glowOffset,
        yOffset = glowOffset,
      })
      -- Scale the proc glow frame and set intensity on child textures
      ...
      if glowFrame then
        ...
        local glowAlpha = glowCfg.alpha or 1.0
        if glowFrame.ProcLoop then
          glowFrame.ProcLoop:SetAlpha(glowAlpha)
        end
        if glowFrame.ProcStart then
          glowFrame.ProcStart:SetAlpha(glowAlpha)
        end
      end
    end
...
local MODULE_BASE_BORDER_OFFSET = 8
local MODULE_PANDEMIC_EXTRA_OFFSET = 5  -- Increase pandemic expansion to align visually with debuff border
...
local function ModuleApplyBorderSizing(borderFrame, iconFrame, pad, zm, frameType)
  ...
  local baseOffset = MODULE_BASE_BORDER_OFFSET
  -- Increase expansion for pandemic to align visually with debuff border
  if frameType == "pandemic" then
    baseOffset = baseOffset + MODULE_PANDEMIC_EXTRA_OFFSET
  end
  -- Simple expansion: base offset minus padding (padding pulls icon inward, so border should be tighter)
  local expand = baseOffset - (pad or 0)
...
local function ModuleEnableBorderFrame(borderFrame, iconFrame, pad, zm, frameType)
  ...
  -- Restore .FX child frame and restart animations (for PandemicIcon)
  if borderFrame.FX then
    borderFrame.FX:SetAlpha(1)
    borderFrame.FX:Show()
    
    -- Restore FX textures (01, 02, 03)
    for i = 1, 3 do
      local tex = borderFrame.FX[string.format("%02d", i)]
      if tex then
        tex:SetAlpha(0) -- Start at 0, animation will fade in
        tex:Show()
      end
    end
    
    -- Restart animation groups
    local animGroups = { borderFrame.FX:GetAnimationGroups() }
...
local function ModuleDisableBorderFrame(borderFrame, frameType)
  ...
  -- Hide .FX child frame and stop animations (for PandemicIcon)
  if borderFrame.FX then
    borderFrame.FX:Hide()
    ...
    if not borderFrame.FX._arcShowHooked then
      ...
      local function EnforceFXHidden(self)
        ...
        if parent then
          ...
          if grandparent and not grandparent._arcShowPandemic then
            self:Hide()
            self:SetAlpha(0)
            local groups = { self:GetAnimationGroups() }
            for _, ag in ipairs(groups) do ag:Stop() end
            return true
          end
          -- Then check parent's _arcShowEnabled
          if parent._arcShowEnabled == false then
      ...
      hooksecurefunc(borderFrame.FX, "Show", function(self)
        ...
        if parent and parent._arcShowEnabled == true then
          self:SetAlpha(1)
          for i = 1, 3 do
            local tex = self[string.format("%02d", i)]
...
ApplyIconStyle = function(frame, cdID)
  ...
  -- OnUpdate watcher: sync settings and catch dynamically created frames
  if not frame._arcBorderWatcher then
    frame._arcBorderWatcher = true
    local checkCount = 0
    local lastPandemicIcon = nil
    frame:HookScript("OnUpdate", function(self, elapsed)
      -- Check immediately if PandemicIcon just appeared (don't wait 5 frames)
      local newPandemic = (self.PandemicIcon and self.PandemicIcon ~= lastPandemicIcon)
      lastPandemicIcon = self.PandemicIcon
      
      -- CRITICAL FIX: When new pandemic frame detected, immediately apply settings
      -- Don't wait for the throttle - CDM may have already shown it
      if newPandemic then
        local pi = self.PandemicIcon
        if not self._arcShowPandemic then
          -- Pandemic should be hidden - immediately disable this new frame
          ModuleDisableBorderFrame(pi, "pandemic")
        end
      end
      
      if not newPandemic then
        checkCount = checkCount + 1
        if checkCount < 30 then return end  -- ~2Hz at 60fps (was 5 = ~12Hz)
        checkCount = 0
      end
      ...
      -- Sync PandemicIcon (handle pooled frames)
      if self.PandemicIcon then
        local pi = self.PandemicIcon
        -- CRITICAL: Check if this is a fresh pooled frame without our hooks
        -- Fresh frames have nil _arcBorderHooked_pandemic - ALWAYS re-apply settings
        local needsHooks = not pi._arcBorderHooked_pandemic
        
        if self._arcShowPandemic then
          -- Should be enabled - apply if not already set up for THIS parent with SAME zoom/padding
          if needsHooks or pi._arcShowEnabled ~= true or pi._arcSizedForParent ~= self or
             pi._arcSizedWithZoom ~= zm or pi._arcSizedWithPadding ~= pad then
            ModuleEnableBorderFrame(pi, self, pad, zm, "pandemic")
          end
        else
          -- Should be disabled - ALWAYS enforce when disabled or missing hooks
          if needsHooks or pi._arcShowEnabled ~= false or pi:IsShown() then
            ModuleDisableBorderFrame(pi, "pandemic")
          end
        end
      end
      
      -- Sync DebuffBorder (handle dynamically created frames)
      if self.DebuffBorder then
        local db = self.DebuffBorder
        -- Check for fresh pooled frame without hooks
        local needsHooks = not db._arcBorderHooked_debuff
        
        if self._arcShowDebuffBorder then
          -- Should be enabled - apply if not already set up for THIS parent with SAME zoom/padding
          if needsHooks or db._arcShowEnabled ~= true or db._arcSizedForParent ~= self or
             db._arcSizedWithZoom ~= zm or db._arcSizedWithPadding ~= pad then
            ModuleEnableBorderFrame(db, self, pad, zm, "debuff")
          end
        else
          -- Should be disabled - always enforce when disabled or missing hooks
          if needsHooks or db._arcShowEnabled ~= false or db:IsShown() then
            ModuleDisableBorderFrame(db, "debuff")
          end
        end
      end
      
      -- Enforce CooldownFlash hiding if configured
      if self._arcHideCooldownFlash and self.CooldownFlash then
        self.CooldownFlash:SetAlpha(0)
        if self.CooldownFlash.Flipbook then
          self.CooldownFlash.Flipbook:SetAlpha(0)
        end
      end
    end)
  end
  ...
  -- Apply SpellActivationAlert (highlight/proc glow) positioning to match Icon padding
  -- This is the yellow glow frame that appears when buff is active (size 46x46 normally)
  -- Must follow padding or glow will be oversized when padding > 0
  if frame.SpellActivationAlert then
    -- Apply padding - glow should surround the padded icon area, not the full frame
    -- Use negative padding to inset from frame edges, matching where icon is positioned
    frame.SpellActivationAlert:ClearAllPoints()
    frame.SpellActivationAlert:SetPoint("TOPLEFT", frame, "TOPLEFT", padding, -padding)
    frame.SpellActivationAlert:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -padding, padding)
    
    -- Hook SetSize so CDM can't override our positioning
    if not frame.SpellActivationAlert._arcSizeHooked then
      frame.SpellActivationAlert._arcSizeHooked = true
      frame.SpellActivationAlert._arcParentFrame = frame
      hooksecurefunc(frame.SpellActivationAlert, "SetSize", function(self)
        local parent = self._arcParentFrame
        if not parent then return end
        local pad = parent._arcPadding or 0
        if pad > 0 then
          self:ClearAllPoints()
          self:SetPoint("TOPLEFT", parent, "TOPLEFT", pad, -pad)
          self:SetPoint("BOTTOMRIGHT", parent, "BOTTOMRIGHT", -pad, pad)
        end
      end)
      hooksecurefunc(frame.SpellActivationAlert, "SetAllPoints", function(self)
        local parent = self._arcParentFrame
        if not parent then return end
        local pad = parent._arcPadding or 0
        if pad > 0 then
  ...
  if frame.Cooldown then
    ...
    -- This ensures the swipe covers the same visual area as the cropped icon
    if frame._arcTexCoords and frame.Cooldown.SetTexCoordRange then
      local tc = frame._arcTexCoords
      -- SetTexCoordRange takes two Vector2D: low (left, top) and high (right, bottom)
    ...
    -- This catches CDMGroups or any other system resizing the frame
  ...
  -- ═══════════════════════════════════════════════════════════════════
  -- RANGE INDICATOR - Hook CDM's RefreshIconColor to prevent flickering
  -- ═══════════════════════════════════════════════════════════════════
  if frame.Icon then
    ...
    if rangeCfg then
      ...
      -- Cleanup old mask texture if it exists from previous version
      if frame._arcRangeMask then
        frame._arcRangeMask:Hide()
        frame._arcRangeMask:SetAlpha(0)
      end
      
      -- Hook RefreshIconColor to intercept CDM's range coloring at the source
        ...
        hooksecurefunc(frame, "RefreshIconColor", function(self)
          ...
          if rCfg.enabled == false then
            ...
            if self.OutOfRange then
              self.OutOfRange:SetAlpha(0)
            end
            -- Only push white if spell IS out of range (to counteract CDM's red)
            if self.spellOutOfRange and self.Icon then
              if ArcUI_CDMEnhance_TintDebug and self.cooldownID == 148700 then
                print("|cff00FFFF[TINT]|r RefreshIconColor cdID=" .. tostring(self.cooldownID) .. " -> pushing WHITE (out of range)")
              end
              self.Icon:SetVertexColor(1, 1, 1, 1)
            end
          else
            -- Range indicator ENABLED - apply custom alpha if out of range
            if self.spellOutOfRange and self.OutOfRange then
              local alpha = rCfg.alpha or 0.6
              self.OutOfRange:SetAlpha(alpha)
            end
          end
        end)
      end
      
      -- Hook OutOfRange:Show to immediately counteract CDM's red tint
      -- When OutOfRange is shown, CDM applies red vertex color - we push white to undo it
      -- This is cleaner than constantly fighting SetVertexColor calls
      if frame.OutOfRange and not frame.OutOfRange._arcHooked then
        ...
        hooksecurefunc(frame.OutOfRange, "Show", function(self)
          ...
          if rCfg and rCfg.enabled == false then
            ...
            if parent.Icon then
              if ArcUI_CDMEnhance_TintDebug and parent.cooldownID == 148700 then
                print("|cff00FFFF[TINT]|r OutOfRange:Show cdID=" .. tostring(parent.cooldownID) .. " -> pushing WHITE")
              end
      ...
      if not frame.RefreshIconColor then
        if not frame._arcRangeWatcher then
          ...
          frame._arcRangeWatcher:SetScript("OnUpdate", function(self, elapsed)
            ...
            if self.throttle < 0.05 then return end  -- 20Hz matches other maintainers
            self.throttle = 0
            ...
            if rCfg.enabled == false then
              if parent.OutOfRange then
                parent.OutOfRange:SetAlpha(0)
              end
              -- Push white when out of range to counteract CDM's red tint
              if parent.Icon and parent.spellOutOfRange then
                if ArcUI_CDMEnhance_TintDebug and parent.cooldownID == 148700 and not parent._arcTintDebugPrinted then
                  print("|cff00FFFF[TINT]|r RangeWatcher cdID=" .. tostring(parent.cooldownID) .. " -> pushing WHITE (out of range)")
                  parent._arcTintDebugPrinted = true
                end
                parent.Icon:SetVertexColor(1, 1, 1, 1)
              elseif not parent.spellOutOfRange then
                parent._arcTintDebugPrinted = nil  -- Reset so we print again next time
              end
            end
          end)
        end
  ...
  -- Simple approach: Hook SpellActivationAlert Show/Hide
  -- - For Blizzard glow: Just apply color, let CDM handle everything else
  -- - For custom glows: Hide CDM glow, apply LCG to icon frame (auto-sizes)
  -- ═══════════════════════════════════════════════════════════════════
  local glowCfg = cfg.procGlow
  if glowCfg then
    frame._arcGlowCfg = glowCfg
    
    -- Use module-level LCG (defined at line 377)
    
    -- ═══════════════════════════════════════════════════════════════
    -- HELPER: Hide CDM's glow (for custom glows only)
    -- ═══════════════════════════════════════════════════════════════
    local function HideBlizzardGlow(cdmGlow)
      if not cdmGlow then return end
      -- Hide all CDM glow children
      if cdmGlow.ProcLoopFlipbook then pcall(function() cdmGlow.ProcLoopFlipbook:Hide() end) end
      if cdmGlow.ProcStartFlipbook then pcall(function() cdmGlow.ProcStartFlipbook:Hide() end) end
      if cdmGlow.ProcAltGlow then pcall(function() cdmGlow.ProcAltGlow:Hide() end) end
      if cdmGlow.ProcLoop then pcall(function() cdmGlow.ProcLoop:Hide() end) end
      if cdmGlow.ProcStartAnim then pcall(function() cdmGlow.ProcStartAnim:Hide() end) end
    end
    
    -- ═══════════════════════════════════════════════════════════════
    -- HELPER: Stop all LCG custom glows
    -- ═══════════════════════════════════════════════════════════════
    local function StopCustomGlows(parentFrame, lcg)
      if not lcg or not parentFrame then return end
      pcall(lcg.PixelGlow_Stop, parentFrame, "ArcUI_Glow")
      pcall(lcg.AutoCastGlow_Stop, parentFrame, "ArcUI_Glow")
      pcall(lcg.ButtonGlow_Stop, parentFrame)
      pcall(lcg.ProcGlow_Stop, parentFrame, "ArcUI_Glow")
    end
    
    -- ═══════════════════════════════════════════════════════════════
    -- HELPER: Start LCG custom glow (attaches to icon, auto-sizes)
    -- ═══════════════════════════════════════════════════════════════
    local function StartCustomGlow(parentFrame, gCfg, lcg)
      if not lcg or not parentFrame then return end
      
      local glowType = gCfg.glowType or "proc"
      local color = {0.95, 0.95, 0.32, gCfg.alpha or 1.0}
      if gCfg.color then
        color = {gCfg.color.r or 1, gCfg.color.g or 1, gCfg.color.b or 1, gCfg.alpha or 1.0}
      end
      
      -- LCG glows attach to icon frame and auto-size
      local padding = 0
      local parentCdID = parentFrame.cooldownID
      local iconCfg = parentCdID and GetIconSettings(parentCdID)
      if iconCfg then padding = iconCfg.padding or 0 end
      local glowOffset = -padding  -- Negative = inward
      
      if glowType == "pixel" then
        local lines = gCfg.lines or 8
        local speed = gCfg.speed or 0.25
        local thickness = gCfg.thickness or 2
        pcall(lcg.PixelGlow_Start, parentFrame, color, lines, speed, nil, thickness, glowOffset, glowOffset, true, "ArcUI_Glow")
      elseif glowType == "autocast" then
        local particles = gCfg.particles or 4
        local speed = gCfg.speed or 0.125
        pcall(lcg.AutoCastGlow_Start, parentFrame, color, particles, speed, 1, glowOffset, glowOffset, "ArcUI_Glow")
      elseif glowType == "button" then
        local speed = gCfg.speed or 0.125
        pcall(lcg.ButtonGlow_Start, parentFrame, color, speed)
      elseif glowType == "proc" then
        pcall(lcg.ProcGlow_Start, parentFrame, {
          color = color,
          startAnim = false,  -- Disable screen-filling start animation
          xOffset = glowOffset,
          yOffset = glowOffset,
          key = "ArcUI_Glow"
        })
        -- Set intensity directly on child textures (ProcGlow vertex color alpha doesn't work well with flipbook)
        local glowFrame = parentFrame["_ProcGlowArcUI_Glow"]
        if glowFrame then
          local glowAlpha = gCfg.alpha or 1.0
          if glowFrame.ProcLoop then
            glowFrame.ProcLoop:SetAlpha(glowAlpha)
          end
          if glowFrame.ProcStart then
    ...
    -- ALWAYS store helper references (needed for glow refresh below)
    frame._arcGlowLCG = LCG
    frame._arcHideBlizzardGlow = HideBlizzardGlow
    frame._arcStopCustomGlows = StopCustomGlows
    frame._arcStartCustomGlow = StartCustomGlow
    
    -- ═══════════════════════════════════════════════════════════════
    -- HOOK: OnSpellActivationOverlayGlowShowEvent
    -- CRITICAL: CDM creates SpellActivationAlert LAZILY - only on first proc!
    -- So we can't hook SpellActivationAlert directly at load time.
    -- This event fires BEFORE CDM shows glow, so we hook it here and then
    -- install our SpellActivationAlert hooks after CDM creates it.
    -- ═══════════════════════════════════════════════════════════════
    if frame.OnSpellActivationOverlayGlowShowEvent and not frame._arcGlowEventHooked then
      frame._arcGlowEventHooked = true
      
      hooksecurefunc(frame, "OnSpellActivationOverlayGlowShowEvent", function(self)
        -- CDM is about to show glow - wait a frame for SpellActivationAlert to be created
        C_Timer.After(0, function()
          if not self or not self._arcStyled then return end
          
          -- Now SpellActivationAlert should exist - hook it if not already done
          if self.SpellActivationAlert and not self.SpellActivationAlert._arcGlowHooked then
            local cdmGlow = self.SpellActivationAlert
            cdmGlow._arcGlowHooked = true
            cdmGlow._arcParentIcon = self
            
            -- Apply padding to newly created SpellActivationAlert
            local pad = self._arcPadding or 0
            if pad > 0 then
              cdmGlow:ClearAllPoints()
              cdmGlow:SetPoint("TOPLEFT", self, "TOPLEFT", pad, -pad)
              cdmGlow:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", -pad, pad)
            end
            
            -- Hook SetSize/SetAllPoints so CDM can't override our positioning
            if not cdmGlow._arcSizeHooked then
              cdmGlow._arcSizeHooked = true
              cdmGlow._arcParentFrame = self
              hooksecurefunc(cdmGlow, "SetSize", function(glow)
                local parent = glow._arcParentFrame
                if not parent then return end
                local p = parent._arcPadding or 0
                if p > 0 then
                  glow:ClearAllPoints()
                  glow:SetPoint("TOPLEFT", parent, "TOPLEFT", p, -p)
                  glow:SetPoint("BOTTOMRIGHT", parent, "BOTTOMRIGHT", -p, p)
                end
              end)
              hooksecurefunc(cdmGlow, "SetAllPoints", function(glow)
                local parent = glow._arcParentFrame
                if not parent then return end
                local p = parent._arcPadding or 0
                if p > 0 then
            ...
            hooksecurefunc(cdmGlow, "Show", function(glow)
              local parentIcon = glow._arcParentIcon
              if not parentIcon or not parentIcon._arcStyled then return end
              
              local freshCfg = GetEffectiveIconSettingsForFrame(parentIcon)
              local gCfg = freshCfg and freshCfg.procGlow
              if not gCfg then return end
              
              parentIcon._arcGlowCfg = gCfg
              local lcg = parentIcon._arcGlowLCG
              
              if parentIcon._arcHideBlizzardGlow then
                parentIcon._arcHideBlizzardGlow(glow)
              end
              
              if gCfg.enabled == false then
                if lcg and parentIcon._arcStopCustomGlows then
                  parentIcon._arcStopCustomGlows(parentIcon, lcg)
                end
                parentIcon._arcCustomGlowActive = false
              else
                if lcg then
                  if parentIcon._arcStopCustomGlows then
                    parentIcon._arcStopCustomGlows(parentIcon, lcg)
                  end
                  if parentIcon._arcStartCustomGlow then
                    parentIcon._arcStartCustomGlow(parentIcon, gCfg, lcg)
                  end
                  parentIcon._arcCustomGlowActive = true
                end
              end
            end)
            
            hooksecurefunc(cdmGlow, "Hide", function(glow)
              local parentIcon = glow._arcParentIcon
          ...
          -- Apply our glow NOW since the event just fired
          if self.SpellActivationAlert then
            local ok, shown = pcall(function() return self.SpellActivationAlert:IsShown() end)
            if ok and shown then
              local freshCfg = GetEffectiveIconSettingsForFrame(self)
              local gCfg = freshCfg and freshCfg.procGlow
              
              if gCfg then
                self._arcGlowCfg = gCfg
                local lcg = self._arcGlowLCG
                
                if self._arcHideBlizzardGlow then
                  self._arcHideBlizzardGlow(self.SpellActivationAlert)
                end
                
                if gCfg.enabled ~= false and lcg then
                  if self._arcStopCustomGlows then
                    self._arcStopCustomGlows(self, lcg)
                  end
                  if self._arcStartCustomGlow then
                    self._arcStartCustomGlow(self, gCfg, lcg)
                  end
                  self._arcCustomGlowActive = true
                end
              end
            end
          end
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- HOOK: SpellActivationAlert Show/Hide (FALLBACK for frames where it already exists)
    -- ═══════════════════════════════════════════════════════════════
    if frame.SpellActivationAlert and not frame.SpellActivationAlert._arcGlowHooked then
      local cdmGlow = frame.SpellActivationAlert
      cdmGlow._arcGlowHooked = true
      cdmGlow._arcParentIcon = frame
      
      hooksecurefunc(cdmGlow, "Show", function(self)
        local parentIcon = self._arcParentIcon
        if not parentIcon or not parentIcon._arcStyled then return end
        
        -- Get FRESH config from frame-level cache
        local freshCfg = GetEffectiveIconSettingsForFrame(parentIcon)
        local gCfg = freshCfg and freshCfg.procGlow
        
        if not gCfg then return end
        
        -- Update cached reference with fresh config
        parentIcon._arcGlowCfg = gCfg
        
        local lcg = parentIcon._arcGlowLCG
        
        -- Stop any preview glow
        if lcg and parentIcon._arcGlowPreviewActive then
          pcall(lcg.PixelGlow_Stop, parentIcon, "ArcUI_Preview")
          pcall(lcg.AutoCastGlow_Stop, parentIcon, "ArcUI_Preview")
          pcall(lcg.ButtonGlow_Stop, parentIcon)
          pcall(lcg.ProcGlow_Stop, parentIcon, "ArcUI_Preview")
          parentIcon._arcGlowPreviewActive = false
        end
        
        -- Always hide CDM's glow - we use LCG for everything
        if parentIcon._arcHideBlizzardGlow then
          parentIcon._arcHideBlizzardGlow(self)
        end
        
        if gCfg.enabled == false then
          -- DISABLED: Stop LCG too
          if lcg and parentIcon._arcStopCustomGlows then
            parentIcon._arcStopCustomGlows(parentIcon, lcg)
          end
          parentIcon._arcCustomGlowActive = false
        else
          -- ALWAYS restart glow with current settings (stop first, then start)
          -- This ensures we pick up any config changes from DB
          if lcg then
            if parentIcon._arcStopCustomGlows then
              parentIcon._arcStopCustomGlows(parentIcon, lcg)
            end
      ...
      hooksecurefunc(cdmGlow, "Hide", function(self)
        local parentIcon = self._arcParentIcon
        if not parentIcon then return end
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- REFRESH/APPLY GLOW: Apply our glow settings when CDM's glow is active
    -- This handles: reload, spec change, opening options, etc.
    -- ═══════════════════════════════════════════════════════════════
    local cdmGlowActive = false
    if frame.SpellActivationAlert then
      local ok, shown = pcall(function() return frame.SpellActivationAlert:IsShown() end)
      cdmGlowActive = ok and shown
    end
    
    if cdmGlowActive and LCG then
      -- CDM's glow is showing - apply our settings
      -- First stop any existing custom glow
      if frame._arcStopCustomGlows then
        frame._arcStopCustomGlows(frame, LCG)
      end
      
      -- Hide CDM's glow (we use our own)
      if frame._arcHideBlizzardGlow and frame.SpellActivationAlert then
        frame._arcHideBlizzardGlow(frame.SpellActivationAlert)
      end
      
      -- Start our custom glow if enabled
      if glowCfg.enabled ~= false then
        if frame._arcStartCustomGlow then
          frame._arcStartCustomGlow(frame, glowCfg, LCG)
          frame._arcCustomGlowActive = true
        end
      else
        frame._arcCustomGlowActive = false
      end
    elseif frame._arcCustomGlowActive and LCG then
      -- CDM's glow is NOT showing but our flag says we have one - stop it
      if frame._arcStopCustomGlows then
        frame._arcStopCustomGlows(frame, LCG)
      end
      frame._arcCustomGlowActive = false
      ...
      if eventType == 1 then -- Available (Aura applied / Cooldown ready)
        ...
        if ev then
          ...
          if ev.showGlow and LCG then
            ...
            pcall(LCG.PixelGlow_Start, self, color, 8, 0.25, nil, 2, glowOffset, glowOffset, true, "ArcUI_Alert", 1)
          end
        end
      ...
      elseif eventType == 2 then -- PandemicTime (Aura at 30% remaining)
        ...
        if ev then
          ...
          if ev.showGlow and LCG then
            ...
            pcall(LCG.PixelGlow_Start, self, color, 8, 0.15, nil, 2, glowOffset, glowOffset, true, "ArcUI_Alert", 1)
          end
        end
      ...
      elseif eventType == 3 then -- OnCooldown (Aura expired / Cooldown used)
        ...
        if ev then
          ...
          if ev.stopGlow and LCG then
            pcall(LCG.PixelGlow_Stop, self, "ArcUI_Alert")
          end
        end
        -- NOTE: Inactive state (desaturation/hide/dim) is handled by CDMGroups via ApplyIconVisuals
        -- Do NOT duplicate that logic here - it causes conflicts when settings change
  ...
  if frame._arcPendingGlowRefresh then
    ...
    if frame.SpellActivationAlert and frame.SpellActivationAlert:IsShown() then
      -- Re-trigger the glow styling using stored functions
      C_Timer.After(0, function()
        if frame and frame.SpellActivationAlert and frame.SpellActivationAlert:IsShown() then
          local cdmGlow = frame.SpellActivationAlert
          local gCfg = frame._arcGlowCfg
          local lcg = frame._arcGlowLCG
          
          if gCfg then
            -- Always hide CDM's glow - we use LCG for everything
            if frame._arcHideBlizzardGlow then frame._arcHideBlizzardGlow(cdmGlow) end
            
            if gCfg.enabled == false then
              if lcg and frame._arcStopCustomGlows then frame._arcStopCustomGlows(frame, lcg) end
            else
              if lcg and frame._arcStartCustomGlow then
                frame._arcStartCustomGlow(frame, gCfg, lcg)
                frame._arcCustomGlowActive = true
              end
            end
          end
...
function SetupChargeText(frame, cdID, cfg)
    ...
    if chargeFrame then
      ...
      if not chargeFrame._arcChargeHideHooked then
        ...
        hooksecurefunc(chargeFrame, "Show", function(self)
          ...
          if not self._arcIsAuraFrame then
            local parentFrame = self._arcParentIconFrame
            if parentFrame and parentFrame.cooldownInfo then
              local spellID = parentFrame.cooldownInfo.overrideSpellID or parentFrame.cooldownInfo.spellID
              if spellID then
                local chargeInfo
                pcall(function() chargeInfo = C_Spell.GetSpellCharges(spellID) end)
                if not chargeInfo then return end  -- Non-charge cooldown, let CDM control
              end
            end
          end
  ...
  -- Let CDM control visibility (it hides at 0/1 stacks automatically)
  -- When re-enabling, we need to Show() the frame ONLY if WE previously hid it
  if chargeFrame then
    ...
    if chargeFrame._arcChargeHideHooked then
      chargeFrame._arcCdID = cdID
      -- ONLY show if we previously hooked and hid this frame
      -- Don't call Show() for frames we never hid - CDM controls visibility
      -- for spells without charges (it hides ChargeCount for non-charge spells)
      chargeFrame:Show()
    end
    -- If no hook exists, don't call Show() - let CDM control visibility
  end
...
local function EnsureGlowHooked(frame, glowSettings)
  ...
  -- Helper to set glow frame level below text overlay (+10) and stacks (+50)
  ...
  local function SetGlowFrameLevel(glowFrame)
    if glowFrame and glowFrame.SetFrameLevel then
      local baseLevel = frame:GetFrameLevel()
      local targetLevel = baseLevel + 1  -- Just above icon, below text (+10) and stacks (+50)
  ...
  if glowType == "pixel" then
    pcall(LCG.PixelGlow_Start, frame, color, lines, speed, nil, thickness, finalXOffset, finalYOffset, true, "ArcUI_ReadyGlow")
    ...
    return glowFrame
  elseif glowType == "autocast" then
    pcall(LCG.AutoCastGlow_Start, frame, color, particles, speed, scale, finalXOffset, finalYOffset, "ArcUI_ReadyGlow")
  ...
  elseif glowType == "proc" then
    ...
    if glowFrame then
      ...
      if glowFrame.ProcStart then
        glowFrame.ProcStart:SetAlpha(intensity)
      end
      if not glowFrame._arcAlphaHooked then
        HookGlowAlpha(glowFrame, frame)
      end
...
HideReadyGlow = function(frame)
  ...
  -- Explicitly hide all possible glow frame types
  if frame._ButtonGlow then
  ...
  -- Hide pixel glow frame (no dash in name!)
  ...
  -- Hide autocast glow frame (no dash in name!)
  ...
  -- Hide proc glow frame (no dash in name!)
  ...
  if frame.PandemicIcon and not frame._arcShowPandemic then
    ...
    if frame.PandemicIcon.FX then
      ...
      local groups = { frame.PandemicIcon.FX:GetAnimationGroups() }
      for _, ag in ipairs(groups) do pcall(function() ag:Stop() end) end
    end
  end
...
ApplyCooldownStateVisuals = function(frame, cfg, normalAlpha, stateVisuals)
  ...
  local useTotemLogic = false
  -- cdID already declared above
  local spellID = cfg._spellID
  
  -- SIMPLIFIED DETECTION: Check totemData first, then auraInstanceID
  -- If frame has totemData with valid slot → use totem logic
  -- If frame has auraInstanceID → use aura logic
  -- This handles Consecration (totemData), Healing Stream (totemData), and normal buffs (auraInstanceID)
  -- OPTIMIZATION: Only check totemData for aura frames (totems are always category 2)
  local hasTotemData, isTotemActive, totemSlot = false, false, nil
  if useAuraLogic then
    hasTotemData, isTotemActive, totemSlot = GetTotemState(frame)
  end
  ...
  if hasTotemData then
    useTotemLogic = true
    useAuraLogic = false
  elseif hasAuraID and useAuraLogic then
    useTotemLogic = false
    -- useAuraLogic stays true
  end
  ...
  -- ═══════════════════════════════════════════════════════════════════
  -- TOTEM LOGIC - active state based on frame.totemData presence
  -- Totems are category 2 but hasAura=false (they're not buffs)
  -- ═══════════════════════════════════════════════════════════════════
  if useTotemLogic then
    -- isTotemActive is determined by totemData.slot being a valid number
    local isReady = isTotemActive
    ...
    if frame._arcTargetDesat == nil then
      if isReady then
        ...
        -- Sync border to ready state (not desaturated)
        ApplyBorderDesaturation(frame, 0)
      else
        -- Inactive state desaturation
        frame._arcBypassDesatHook = true
  ...
  -- ═══════════════════════════════════════════════════════════════════
  -- AURA LOGIC (category 2/3) - non-secret auraInstanceID
  -- ═══════════════════════════════════════════════════════════════════
  if useAuraLogic then
    local auraID = frame.auraInstanceID
...
function ns.CDMEnhance.ShowProcGlowPreview(cdID)
  ...
  local glowType = glowCfg and glowCfg.glowType or "proc"
  ...
  if glowType == "pixel" then
    pcall(LCG.PixelGlow_Start, frame, colorTbl, lines, speed, nil, thickness, xOffset, yOffset, true, "ArcUI_ProcPreview")
  elseif glowType == "autocast" then
    pcall(LCG.AutoCastGlow_Start, frame, colorTbl, particles, speed, scale, xOffset, yOffset, "ArcUI_ProcPreview")
  elseif glowType == "button" then
    pcall(LCG.ButtonGlow_Start, frame, colorTbl, speed)
  else -- proc (default)
    pcall(LCG.ProcGlow_Start, frame, {
      color = colorTbl,
      startAnim = true,
...
function ns.CDMEnhance.RefreshProcGlow(cdID)
  ...
  if not frame._arcCustomGlowActive then return end
  ...
  if glowType == "pixel" then
    pcall(LCG.PixelGlow_Start, frame, colorTbl, lines, speed, nil, thickness, glowOffset, glowOffset, true, "ArcUI_Glow")
  elseif glowType == "autocast" then
    pcall(LCG.AutoCastGlow_Start, frame, colorTbl, particles, speed, scale, glowOffset, glowOffset, "ArcUI_Glow")
  elseif glowType == "button" then
    pcall(LCG.ButtonGlow_Start, frame, colorTbl, speed)
  else -- proc (default)
    pcall(LCG.ProcGlow_Start, frame, {
    ...
    -- Set intensity directly on child textures
    local glowFrame = frame["_ProcGlowArcUI_Glow"]
    if glowFrame then
      if glowFrame.ProcLoop then
        glowFrame.ProcLoop:SetAlpha(alpha)
      end
...
function ns.CDMEnhance.OptimizedApplyIconVisuals(frame)
  ...
  -- Use cached _isAura from config (set during GetEffectiveIconSettings)
  local isTotem, isTotemActive = GetTotemState(frame)
  local isAura = cfg._isAura
  ...
  elseif isAura then
    local auraID = frame.auraInstanceID
    isReady = auraID and type(auraID) == "number" and auraID > 0
  ...
  -- Only set alpha if target changed
  if frame._arcTargetAlpha ~= targetAlpha then
    frame._arcTargetAlpha = targetAlpha
    frame._arcBypassFrameAlphaHook = true
    frame:SetAlpha(targetAlpha)
    if frame.Cooldown then frame.Cooldown:SetAlpha(targetAlpha) end
  ...
  -- Only set desaturation if target changed
  if iconTex and frame._arcTargetDesat ~= targetDesat then
    frame._arcTargetDesat = targetDesat
    frame._arcBypassDesatHook = true
    if iconTex.SetDesaturation then
...
-- fields can be top-level keys or "subtable.key" format
function ns.CDMEnhance.HasSectionCustomizations(cdID, fields)
  ...
  for _, field in ipairs(fields) do
    ...
    if subtable and subkey then
      if raw[subtable] and raw[subtable][subkey] ~= nil then
        return true
      end
    else
      -- Top-level field
...
function ns.CDMEnhance.InvalidateCache()
  ...
  -- Clear cached alpha/desat/tint/glow and cfg on all frames so they recalculate with new settings
  for cdID, data in pairs(enhancedFrames) do
    if data and data.frame then
...
function ns.CDMEnhance.SetIconPosition(cdID, x, y)
  ...
  if ns.CDMGroups and ns.CDMGroups.freeIcons then
    ...
    if freeData then
      ...
      -- FIXED: Use SaveFreeIconToSpec which writes to profile.freeIcons (correct location)
      -- Previously this wrote to specData.freeIcons which is LEGACY and not persisted
...
-- Detects when options panel opens/closes to refresh preview alpha
-- ===================================================================
local optionsPanelTracker = CreateFrame("Frame")
local lastOptionsPanelState = false

optionsPanelTracker:SetScript("OnUpdate", function(self, elapsed)
  -- Check panel state periodically (every frame is fine, it's cheap)
  local currentState = ns.CDMEnhance.IsOptionsPanelOpen and ns.CDMEnhance.IsOptionsPanelOpen() or false
  
  if currentState ~= lastOptionsPanelState then
    lastOptionsPanelState = currentState
...
-- Update all cooldown icons when any cooldown changes
-- We can't trust the payload spellID (might be secret in combat)
-- So we loop through all tracked cooldown frames
local function OnCooldownEvent()
  ...
  -- Loop through all enhanced frames
  for cdID, data in pairs(enhancedFrames) do
    if data.viewerType == "cooldown" or data.viewerType == "utility" then
      local frame = data.frame
      if frame and frame._arcCachedSpellID then
        ...
        -- Apply visuals with fresh duration
        local cfg = GetEffectiveIconSettingsForFrame(frame)
        if cfg then
          local stateVisuals = GetEffectiveStateVisuals(cfg)
          if stateVisuals or frame._arcIgnoreAuraOverride then
            ApplyCooldownStateVisuals(frame, cfg, cfg.alpha or 1.0, stateVisuals)
            -- Mark as event-driven so 10Hz skips it
  ...
  -- Also handle CDMGroups managed icons
  if ns.CDMGroups and ns.CDMGroups.groups then
    for groupName, group in pairs(ns.CDMGroups.groups) do
      if group.members then
        for cdID, member in pairs(group.members) do
          if member and member.frame and (member.viewerType == "cooldown" or member.viewerType == "utility") then
            local frame = member.frame
            if frame._arcCachedSpellID then
              local spellID = frame._arcCachedSpellID
...
cooldownEventFrame:SetScript("OnEvent", function(self, event)
  ...
  C_Timer.After(0.01, OnCooldownEvent)
end)
